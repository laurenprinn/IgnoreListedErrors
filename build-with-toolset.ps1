...
.DESCRIPTION
	Builds the IgnoreListedErrors CMake project with various Visual Studio toolsets
...
Get-ChildItem -Path $buildDir -Filter "*.vcxproj" -Recurse | ForEach-Object {
	...
	$content = $content -replace '<WindowsTargetPlatformVersion>.*?</WindowsTargetPlatformVersion>', "<WindowsTargetPlatformVersion>$SdkVersion</WindowsTargetPlatformVersion>"
...
if ($LASTEXITCODE -eq 0) {
	...
	$exePath = "$buildDir\IgnoreListedErrors\$BuildType\IgnoreListedErrors.exe"
